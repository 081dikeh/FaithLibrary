...
function Pagination({ current, total, tab }: { current: number; total: number; tab: string }) {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, paddingTop: 24 }}>
      {current > 1 && (
        <a href={build(current - 1)} style={pageBtnStyle(false)}><ChevronLeft size={14} /></a>
      )}
      {pages.map((p, i) =>
        p === '...'
          ? <span key={`d${i}`} style={{ width: 36, textAlign: 'center', color: 'var(--text-muted)', fontSize: '0.875rem' }}>…</span>
          : <a key={p} href={build(p as number)} style={pageBtnStyle(p === current)}>{p}</a>
      )}
      {current < total && (
        <a href={build(current + 1)} style={pageBtnStyle(false)}><ChevronRight size={14} /></a>
      )}
    </div>
  )
}
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--bone)' }} className="grain">
      ...
      <div style={{ background: 'var(--roasted)', position: 'relative', overflow: 'hidden' }}>
        <div style={{ position: 'absolute', top: -64, right: -64, width: 300, height: 300, borderRadius: '50%', background: 'rgba(93,64,55,0.4)', filter: 'blur(60px)', pointerEvents: 'none' }} />
        <div style={{ position: 'absolute', bottom: 0, left: -24, width: 200, height: 200, borderRadius: '50%', background: 'rgba(141,110,99,0.18)', filter: 'blur(48px)', pointerEvents: 'none' }} />
        <div className="page-container" style={{ position: 'relative', paddingTop: 40, paddingBottom: 40 }}>
          <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'flex-end', justifyContent: 'space-between', gap: 16, marginBottom: 24 }}>
            <div>
              <p className="section-eyebrow" style={{ color: 'var(--ochre)', marginBottom: 4 }}>Welcome back</p>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12 }}
            ...
            {stats.map(stat => (
              <div key={stat.label} style={{
                background: 'rgba(93,64,55,0.35)', border: '1px solid rgba(141,110,99,0.22)',
                borderRadius: 12, padding: '14px 16px',
                display: 'flex', alignItems: 'center', gap: 12,
              }}>
                <div style={{ color: 'var(--ochre)', flexShrink: 0 }}>{stat.icon}</div>
                <div>
                  <div style={{ fontFamily: 'var(--font-display)', fontSize: '1.5rem', fontWeight: 700, color: 'var(--bone)', lineHeight: 1 }}>{stat.value.toLocaleString()}</div>
      ...
      <style>{`.dashboard-stats { @media (max-width: 580px) { grid-template-columns: repeat(2, 1fr) !important; } }`}</style>
            ...
            <div style={{ marginTop: 24 }}>
              {tab === 'collections' ? (
                myCollections.length === 0
                  ? <EmptyState icon={<FolderOpen size={28} />} title="No collections yet" body="Group scores into collections for Mass, seasons, or events." cta="Create Collection" href="/collections" />
                ...
                (tab === 'uploads' ? myUploads : bookmarkedFiles).length === 0
                  ? <EmptyState icon={tab === 'bookmarks' ? <Bookmark size={28} /> : <FileStack size={28} />} title={tab === 'bookmarks' ? 'No bookmarks yet' : 'No uploads yet'} body={tab === 'bookmarks' ? 'Browse the library and bookmark scores to save them here.' : 'Upload your first choral score or hymn to the library.'} cta={tab === 'bookmarks' ? 'Browse Library' : 'Upload Now'} href={tab === 'bookmarks' ? '/' : '/upload'} />
                  : <>
...
function EmptyState({ icon, title, body, cta, href }: { icon: React.ReactNode; title: string; body: string; cta: string; href: string }) {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 12, paddingTop: 80, paddingBottom: 80, textAlign: 'center' }}>
      ...
      <p style={{ fontFamily: 'var(--font-display)', fontSize: '1.25rem', color: 'var(--walnut)', fontWeight: 600 }}>{title}</p>
      <p style={{ fontSize: '0.875rem', color: 'var(--text-muted)', maxWidth: 300, lineHeight: 1.6 }}>{body}</p>
