...
export function NotificationBell() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      // Set up realtime — subscribe BEFORE calling .on()
    ...
    <div ref={containerRef} className="relative">
      ...
      {open && (
        <div className="absolute right-0 top-full mt-2 w-80 z-50
          ...
          {/* Header */}
          <div className="px-4 py-3 border-b border-[#EFE9E7]
            ...
            <div className="flex items-center gap-2">
              ...
              {unread > 0 && (
                <span className="badge badge-walnut">{unread} new</span>
              )}
            </div>
            {unread > 0 && (
              ...
              notifications.map(n => (
                <div key={n.id}
                  className={`flex items-start gap-3 px-4 py-3
                              border-b border-[#EFE9E7] last:border-0
                              ${!n.is_read ? 'bg-[#EFE9E7]/40' : ''}`}>
                    ...
                    {n.link ? (
                      <Link href={n.link} onClick={() => setOpen(false)}
                        className="text-sm font-medium text-[#3E2723]
                                   hover:text-[#5D4037] transition-colors
                                   leading-snug block"
