...
// ─── Singleton render queue ───────────────────────────────────
// Prevents multiple cards from hitting the PDF worker simultaneously.
  ...
  useEffect(() => {
    ...
    enqueue(async () => {
      ...
      try {
        ...
        // One frame so the parent has painted and has a real clientWidth
        ...
        const renderTask = pg.render({ canvasContext: ctx, viewport: vp } as any)
...
export function ScoreCard({ file, bookmarked = false, index = 0 }: ScoreCardProps) {
  ...
  return (
    <article
      className="group relative bg-white rounded-xl overflow-hidden flex flex-col border border-[#D7CCC8] hover:shadow-[0_8px_28px_rgba(62,39,35,0.14)] hover:-translate-y-1 transition-all duration-200"
    >
      <Link href={`/view/${file.id}`} className="block flex-shrink-0" tabIndex={-1} aria-hidden>
        <div
          ref={thumbRef}
          className="relative w-full overflow-hidden bg-[#EFE9E7]"
        ...
        >
          ...
          {file.tags?.[0] && (
            <span
              className="absolute top-2 left-2 z-20 inline-block px-2 py-0.5 rounded-full text-[0.58rem] font-bold uppercase tracking-wide leading-relaxed"
          ...
          <button
            ...
            className={`absolute top-2 right-2 z-20 w-7 h-7 rounded-full flex items-center justify-center shadow-sm transition-all
              ${isBookmarked
                ...
                : 'bg-white/90 text-[#8D6E63] opacity-0 group-hover:opacity-100'
              }`}
          >
      ...
      <div className="flex flex-col flex-1 px-3 pt-2.5 pb-3">
        <Link href={`/view/${file.id}`} className="no-underline">
          <h3
            className="font-display font-semibold text-[#3E2723] text-[0.8125rem] leading-snug group-hover:text-[#5D4037] transition-colors mb-1"
        ...
        {file.composer && (
          <p className="text-[0.7rem] text-[#5D4037] font-medium truncate mb-0.5" style={{ fontFamily: 'var(--font-ui)' }}>
            {file.composer}
          </p>
        )}
        {file.profiles?.full_name && (
          <p className="text-[0.7rem] text-[#8D6E63] truncate mb-0.5" style={{ fontFamily: 'var(--font-ui)' }}>
            {file.profiles.full_name}
          </p>
        )}

        <div className="flex items-center justify-between mt-auto pt-2 border-t border-[#EFE9E7]">
          <span className="text-[0.68rem] text-[#8D6E63] flex items-center gap-1" style={{ fontFamily: 'var(--font-ui)' }}>
            {timeAgo}
            {(file.download_count ?? 0) > 0 && (
              <><span className="opacity-30">·</span><Download size={8} />{file.download_count}</>
            )}
          </span>
          <button
            onClick={handleDownload}
